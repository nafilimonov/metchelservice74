...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/informationsystem/informationsystem_item">
		...
		<xsl:if test="property_value[tag_name='photo' and file/node()]">
			<div class="box">
				<div class="gallery">
					<xsl:apply-templates select="property_value[tag_name='photo']"  mode="photo"/>
				</div>
			</div>
		</xsl:if>
		
		<xsl:if test="property_value[tag_name='file' and file/node()]">
			<div class="box">
				<xsl:apply-templates select="property_value[tag_name='file']"  mode="file"/>
			</div>
		</xsl:if>
	...
	<xsl:template match="property_value" mode="photo">
		<xsl:if test="file_small != ''">
			<xsl:text disable-output-escaping="yes">&lt;div  class="photo_img_cell" &gt;</xsl:text>
			<a href="{../dir}{file}" class="fancybox" rel="gallery" target="_blank" title="{file_description}"><img src="{../dir}{file_small}" /></a>
			<xsl:text disable-output-escaping="yes">&lt;/div&gt;</xsl:text>
		</xsl:if>
	</xsl:template>
	
	<xsl:template match="property_value"  mode="file">
