...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="informationsystem_item">
		...
		<xsl:if test="property_value[tag_name='file']/file != ''">
			...
			<p><img src="/hostcmsfiles/images/icons/{$file_type}" class="img" /><xsl:text> </xsl:text>
				<a href="{dir}{property_value[tag_name='file']/file}" target="_blank"><xsl:value-of select="name"/></a>
			</p>
		</xsl:if>
	...
	<xsl:template name="file_type">
		...
		<xsl:choose>
			...
			<xsl:when test="$sub_str = 'doc' or $sub_str = 'docx' or $sub_str = 'rtf'">doc.gif</xsl:when>
