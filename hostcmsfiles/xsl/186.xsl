...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="shop">
		<xsl:if test="count(affiliate_plan)">
			...
			<p>
				Персональный бонус: <b><xsl:value-of select="format-number(sum(/siteuser/transactions/shop_siteuser_transaction[shop_id = $shop_id]/amount_base_currency), '0.##')" /><xsl:text> </xsl:text><xsl:value-of select="/siteuser/shop[@id = $shop_id]/shop_currency/sign" /></b>
			</p>
