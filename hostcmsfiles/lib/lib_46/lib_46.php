...
$oInformationsystem_phones_group = Core_Entity::factory('Informationsystem', 30);
$aInformationsystem_Groups_phones_group = $oInformationsystem_phones_group->Informationsystem_Groups->findAll();

$oXmlStructureTag = Core::factory('Core_Xml_Entity')->name('phones_groups');

foreach ($aInformationsystem_Groups_phones_group as $oGroup)
{
    $oXmlStructureTag->addEntity(
        Core::factory('Core_Xml_Entity')
            ->name('phones_group')
            ->addAttribute('id', $oGroup->id)
            ->addEntity(
                Core::factory('Core_Xml_Entity')
                    ->name('name')
                    ->value($oGroup->name)
            ...
            ->addEntity(
                Core::factory('Core_Xml_Entity')
                    ->name('phones_group_id')
                    ->value($oGroup->id)
            )
    );
}

$Informationsystem_Controller_Show->addEntity($oXmlStructureTag);
