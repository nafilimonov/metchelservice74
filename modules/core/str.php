...
class Core_Str
{
	...
	static public function cutSentences($text, $maxLen = 255)
	{
		...
		if ($lenght > $maxLen)
		{
			...
			// [words] + space + not-space{3,} + [.!?。]
			preg_match('/^((?:.*?\s[^\s]{3,}[.!?。])*)/su', $text, $matches);
