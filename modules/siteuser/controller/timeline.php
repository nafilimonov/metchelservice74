...
class Siteuser_Controller_Timeline extends Admin_Form_Controller_View
{
	...
	protected function _showContent()
	{
		...
		$additionalParams = 'siteuser_id={siteuser_id}&secret_csrf=' . Core_Security::getCsrfToken();
		...
		?>
		<div>
			<form action="<?php echo Admin_Form_Controller::correctBackendPath('/{admin}/siteuser/timeline/index.php')?>?hostcms[action]=addNote&_=<?php echo time()?>&hostcms[checked][0][1-0]=1&siteuser_id=<?php echo $siteuser_id?>&parentWindowId=<?php echo htmlspecialchars($windowId)?>" method="POST" enctype='multipart/form-data' class="padding-bottom-10 dropzone-form dropzone-form-timeline">
						...
						foreach ($aTmpEntities as $key => $oTmpEntity)
						{
							...
							switch (get_class($oTmpEntity))
							{
								case 'Siteuser_Email_Model':
									$badge = 'fa fa-envelope-o';
									$color = 'azure';

									$text = $oTmpEntity->subjectBackend(NULL, $oAdmin_Form_Controller);

									$iEntityAdminFormId = 276;

									$path = '/{admin}/siteuser/email/index.php';
								break;
								...
								case 'Event_Model':
									...
									$path = '/{admin}/siteuser/timeline/index.php';
								break;
								case 'Deal_Model':
								...
								case 'Crm_Note_Model':
									...
									{
										$files = $oTmpEntity->getFilesBlock($oTmpEntity->Siteuser);
								...
								<div class="timeline-panel">
									...
									<div class="timeline-body">
										...
										<div class="timeline-body-footer small gray">
											...
											?><span class="timeline-user"><?php !is_null($oUserAuthor) ? $oUserAuthor->showLink($oAdmin_Form_Controller->getWindowId()) : ''?></span>
			...
			$form.dropzone({
				...
				init: function() {
					...
					$("#<?php echo $windowId?> .dropzone-form-timeline button#sendForm").on("click", function(e) {
						...
						{
							$form.append('<input type="hidden" name="hostcms[window]" value="<?php echo $windowId?>-siteuser-timeline">');
				...
				success : function(file, response){
					...
					$.adminLoad({ path: hostcmsBackend + '/siteuser/note/index.php', additionalParams: 'siteuser_id=<?php echo $siteuser_id?>', windowId: window_id });
				}
			});
		});
		</script><?php
