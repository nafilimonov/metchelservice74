 ...
 */
class Siteuser_Controller_Email extends Admin_Form_Controller_View
{
	/**
	...
	protected function _showContent()
	{
		...
		if (count($aEntities))
		{
			...
			foreach ($aEntities as $oSiteuser_Email)
			{
				$color = $aColors[$i % $iCountColors];

				$iDatetime = Core_Date::sql2timestamp($oSiteuser_Email->datetime);
				...
				<li class="timeline-inverted">
					<div class="timeline-badge azure">
					...
					<div class="timeline-panel">
						<div class="timeline-header bordered-bottom bordered-palegreen">
							<div class="pull-right timeline-entity-actions">
								...
								if ($oAdmin_Form->show_operations)
								{
									...
									{
										$aAllowedActions = array('send', 'edit', 'markDeleted');
										...
										if (method_exists($oSiteuser_Email, 'checkBackendAccess') && !$oSiteuser_Email->checkBackendAccess($oAdmin_Form_Action->name, $oUser))
										{
											continue;
										}
										...
										$href = $oAdmin_Form_Controller->getAdminActionLoadHref($path, $oAdmin_Form_Action->name, NULL, 0, $oSiteuser_Email->id, $additionalParams, 10, 1, NULL, NULL, 'list');

										$onclick = $oAdmin_Form_Action->name == 'edit'
											? $oAdmin_Form_Controller->getAdminActionModalLoad(array('path' => $path, 'action' => $oAdmin_Form_Action->name, 'operation' => 'modal', 'datasetKey' => 0, 'datasetValue' => $oSiteuser_Email->id, 'additionalParams' => $additionalParams, 'width' => '90%'))
											: $oAdmin_Form_Controller->getAdminActionLoadAjax($path, $oAdmin_Form_Action->name, NULL, 0, $oSiteuser_Email->id, $additionalParams, 10, 1, NULL, NULL, 'list');

										// Добавляем установку метки для чекбокса и строки + добавлем уведомление, если необходимо
										if ($oAdmin_Form_Action->confirm)
										{
											$onclick = "res = confirm('".Core::_('Admin_Form.confirm_dialog', htmlspecialchars($name))."'); if (!res) { $('#{$windowId} #row_0_{$oSiteuser_Email->id}').toggleHighlight(); } else {mainFormLocker.unlock(); {$onclick}} return res;";
										}
						...
						<div class="timeline-body">
							<?php echo $oSiteuser_Email->subjectBackend(NULL, $oAdmin_Form_Controller)?>
							<div class="small gray well-info">
								<?php
								$oCreator = $oSiteuser_Email->User;

								if (!is_null($oCreator))
								{
									?><span class="gray"><?php $oCreator->showLink($oAdmin_Form_Controller->getWindowId())?></span><?php
								}
								?>
