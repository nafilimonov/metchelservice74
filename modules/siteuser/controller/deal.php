 ...
 */
class Siteuser_Controller_Deal extends Admin_Form_Controller_View
{
	/**
	...
	protected function _showContent()
	{
		...
		if (count($aEntities))
		{
			...
			foreach ($aEntities as $oDeal)
			{
				$color = $aColors[$i % $iCountColors];

				$iDatetime = Core_Date::sql2timestamp($oDeal->start_datetime);
				...
				<li class="timeline-inverted">
					...
					<div class="timeline-panel">
							...
							<div class="pull-right timeline-entity-actions">
								...
								if ($oAdmin_Form->show_operations)
								{
									...
									foreach ($aAllowed_Admin_Form_Actions as $oAdmin_Form_Action)
									{
										...
										if (method_exists($oDeal, 'checkBackendAccess') && !$oDeal->checkBackendAccess($oAdmin_Form_Action->name, $oUser))
										{
											continue;
										}
										...
										$href = $oAdmin_Form_Controller->getAdminActionLoadHref($path, $oAdmin_Form_Action->name, NULL, 0, $oDeal->id, $additionalParams, 10, 1, NULL, NULL, 'list');

										$onclick = $oAdmin_Form_Action->name == 'edit'
											? $oAdmin_Form_Controller->getAdminActionModalLoad(array('path' => $path, 'action' => $oAdmin_Form_Action->name, 'operation' => 'modal', 'datasetKey' => 0, 'datasetValue' => $oDeal->id, 'additionalParams' => $additionalParams, 'width' => '90%'))
											: $oAdmin_Form_Controller->getAdminActionLoadAjax($path, $oAdmin_Form_Action->name, NULL, 0, $oDeal->id, $additionalParams, 10, 1, NULL, NULL, 'list');

										// Добавляем установку метки для чекбокса и строки + добавлем уведомление, если необходимо
										if ($oAdmin_Form_Action->confirm)
										{
											$onclick = "res = confirm('".Core::_('Admin_Form.confirm_dialog', htmlspecialchars($name))."'); if (!res) { $('#{$windowId} #row_0_{$oDeal->id}').toggleHighlight(); } else {mainFormLocker.unlock(); {$onclick}} return res;";
										}
						...
						<div class="timeline-body">
							<?php echo $oDeal->showContent($oAdmin_Form_Controller)?>
							<div class="small gray well-info">
								<?php
								$oDealCreator = $oDeal->Creator;

								if (!is_null($oDealCreator))
								{
									?><span class="gray"><?php $oDealCreator->showLink($oAdmin_Form_Controller->getWindowId())?></span><?php
								}
								?>
