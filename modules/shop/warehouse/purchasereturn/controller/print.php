 ...
 */
class Shop_Warehouse_Purchasereturn_Controller_Print extends Printlayout_Controller_Print
{
	protected function _prepare()
	{
		...
		if (!is_null($oPrintlayout))
		{
			...
			if (isset($aChecked[0]))
			{
				$shop_warehouse_purchasereturn_id = key($aChecked[0]);

				$oShop_Warehouse_Purchasereturn = Core_Entity::factory('Shop_Warehouse_Purchasereturn')->getById($shop_warehouse_purchasereturn_id);

				if (!is_null($oShop_Warehouse_Purchasereturn))
				{
					$this->_oPrintlayout_Controller
						->replace($oShop_Warehouse_Purchasereturn->getPrintlayoutReplaces())
						->driver($oPrintlayout_Driver)
						->entity($oShop_Warehouse_Purchasereturn);
				}
			}
		}
