...
/**
 * Shop_Warehouse_Supply Backend Editing Controller.
 *
 * @package HostCMS
 * @subpackage Shop
 * @version 7.x
 ...
 */
class Shop_Warehouse_Supply_Controller_Edit extends Admin_Form_Action_Controller_Type_Edit
{
	/**
	...
	protected function _prepareForm()
	{
		...
		$oSelect_Companies = Admin_Form_Entity::factory('Select')
			...
			->caption(Core::_('Shop_Warehouse_Supply.company_id'))
		...
		if (Core::moduleIsActive('siteuser'))
		{
			...
			$oScriptSiteusers = Admin_Form_Entity::factory('Script')
				->value('
					$("#' . $windowId . ' #supply_siteuser_company_id").select2({
			...
			$oSelectContracts = Admin_Form_Entity::factory('Select')
				...
				->caption(Core::_('Shop_Warehouse_Supply.siteuser_company_contract_id'))
				->divAttr(array('class'=>'col-xs-12'));
		...
		$this->getField('registration_number')
			...
			->add(
				Core_Html_Entity::factory('Span')
					...
					->value(Core::_('Shop_Warehouse_Supply.registration_date_preposition'))
			);
		...
		$oShop_Warehouse_Select = Admin_Form_Entity::factory('Select')
			->caption(Core::_('Shop_Warehouse_Supply.shop_warehouse_id'))
			->divAttr(
		...
		$oSelectResponsibleUsers = Admin_Form_Entity::factory('Select')
			...
			->caption(Core::_('Shop_Warehouse_Supply.user_id'))
			->divAttr(array('class' => ''));
		...
		$oShop_Price_Select = Admin_Form_Entity::factory('Select')
			->caption(Core::_('Shop_Warehouse_Supply.shop_price_id'))
			->divAttr(
				array('class' => 'form-group col-xs-12 col-sm-3')
			)
		...
		$oDivActions->add(
			...
			Admin_Form_Entity::factory('Code')->html(Shop_Warehouse_Controller::createDocumentButton($oAdmin_Form_Controller, $this->_object, array('invoice', 'purchasereturn', 'warrant_order', 'warrant_pay')))
		);
		...
		$itemTable = '
			<div class="table-scrollable">
				<table class="table table-striped table-hover shop-item-table deals-aggregate-user-info">
					<thead>
						<tr>
							<th scope="col">' . Core::_('Shop_Warehouse_Supply.position') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Supply.name') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Supply.measure') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Supply.price') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Supply.currency') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Supply.quantity') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Supply.sum') . '</th>
							<th scope="col"> </th>
						</tr>
					</thead>
					<tbody>
		...
		do {
			...
			if ($createFrom && !$this->_object->id)
			{
				switch ($createFrom)
				{
					case 'purchaseorder':
						...
						$this->_object->description = $oShop_Warehouse_Purchaseorder->description;
					break;
					case 'invoice':
						$oShop_Warehouse_Invoice = Core_Entity::factory('Shop_Warehouse_Invoice', $createFromId);
			...
			{
				$oShop_Warehouse_Supply_Items = $this->_object->Shop_Warehouse_Supply_Items;
				$oShop_Warehouse_Supply_Items->queryBuilder()
					->limit($limit)
					->offset($offset)
					->clearOrderBy()
					->orderBy('shop_warehouse_supply_items.id');
			...
			{
				$oShop_Item = Core_Entity::factory('Shop_Item')->getById($oShop_Warehouse_Supply_Item->shop_item_id);
				...
				if (!is_null($oShop_Item))
				{
					...
					$onclick = !$createFrom || $this->_object->id
						? $oAdmin_Form_Controller->getAdminActionLoadAjax($oAdmin_Form_Controller->getPath(), 'deleteShopItem', NULL, 0, $oShop_Item->id, "shop_warehouse_supply_item_id={$oShop_Warehouse_Supply_Item->id}")
						: '';
					...
					$itemTable .= '
						<tr ' . ( !$createFrom ? ('id="' . $oShop_Warehouse_Supply_Item->id) . '"' : '') . ' data-item-id="' . $oShop_Item->id . '">
							...
							<td width="110"><input type="text" class="price set-item-price form-control" name="shop_item_price' . (!$createFrom ? ('_' . $oShop_Warehouse_Supply_Item->id) : '[]') . '" value="' . $oShop_Warehouse_Supply_Item->price . '" /></td>
		...
		while (!$createFrom && count($aShop_Warehouse_Supply_Items));
		...
		$oShopItemRow1->add(
			Admin_Form_Entity::factory('Input')
				...
				->placeholder(Core::_('Shop_Warehouse_Supply.add_item_placeholder'))
				->name('set_item_name')
		);
		...
		$this->title($this->_object->id
			? Core::_('Shop_Warehouse_Supply.form_edit', $this->_object->number, FALSE)
			: Core::_('Shop_Warehouse_Supply.form_add')
		);
	...
	public function execute($operation = NULL)
	{
		...
		// Создаем приемку из формы редактирования заказа поставщику или счета поставщика
		if ($createFromId && !is_null($oBaseDocument) && !$this->_object->id)
		{
			$oShopWarehouseSupply = $oBaseDocument->getById($createFromId)->createShopWarehouseSupply();

			$this->_object = $oShopWarehouseSupply;
		}
	 ...
	 * @hostcms-event Shop_Warehouse_Supply_Controller_Edit.onAfterRedeclaredApplyObjectProperty
	 */
	protected function _applyObjectProperty()
	{
		...
		{
			$modelName == 'shop_warehouse_supply'
		...
		// Существующие товары
		$aShop_Warehouse_Supply_Items = $this->_object->Shop_Warehouse_Supply_Items->findAll(FALSE);
		foreach ($aShop_Warehouse_Supply_Items as $oShop_Warehouse_Supply_Item)
		{
			$oShop_Item = Core_Entity::factory('Shop_Item')->getById($oShop_Warehouse_Supply_Item->shop_item_id);
			...
			if (!is_null($oShop_Item))
			{
				...
				$quantity = Core_Array::getPost('shop_item_quantity_' . $oShop_Warehouse_Supply_Item->id, 0);

				$oShop_Warehouse_Supply_Item->count != $quantity && $bNeedsRePost = TRUE;

				$oShop_Warehouse_Supply_Item->count = $quantity;
				//$oShop_Warehouse_Purchaseorder_Item->price = $aPrices['price_tax'];
				$oShop_Warehouse_Supply_Item->price = $quantity = Core_Array::getPost('shop_item_price_' . $oShop_Warehouse_Supply_Item->id, 0);
		...
		if (count($aAddShopItems))
		{
			...
			foreach ($aAddShopItems as $key => $shop_item_id)
			{
				...
				if (!is_null($oShop_Item))
				{
					...
					$oShop_Warehouse_Supply_Item
						->shop_warehouse_supply_id($this->_object->id)
						->shop_item_id($oShop_Item->id)
						->count($count)
						->price($aPrices['price_tax'])
						->save();

					$onclick = 'mainFormLocker.unlock(); res = confirm(\'' . Core::_('Shop_Warehouse_Supply.delete_dialog') . '\'); if (res) { var next = $(this).parents(\'tr\').next(); $(this).parents(\'tr\').remove(); $.recountIndexes(next); ' . $oAdmin_Form_Controller->getAdminActionLoadAjax($oAdmin_Form_Controller->getPath(), 'deleteShopItem', NULL, 0, $oShop_Item->id, "shop_warehouse_supply_item_id={$oShop_Warehouse_Supply_Item->id}") . ' } return res;';

					$script .= "aMapShopItemId['{$shop_item_id}'] = {
						id: {$oShop_Warehouse_Supply_Item->id},
						onClickValue: \"{$onclick}\"
					};";

					$script .= "jShopItemPrice.eq({$key}).attr('name', 'shop_item_price_{$oShop_Warehouse_Supply_Item->id}');
					jShopItemQuantity.eq({$key}).attr('name', 'shop_item_quantity_{$oShop_Warehouse_Supply_Item->id}');";
				}
				else
				{
