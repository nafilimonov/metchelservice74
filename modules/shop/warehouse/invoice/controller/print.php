 ...
 */
class Shop_Warehouse_Invoice_Controller_Print extends Printlayout_Controller_Print
{
	protected function _prepare()
	{
		...
		if (!is_null($oPrintlayout))
		{
			...
			if (isset($aChecked[0]))
			{
				$shop_warehouse_invoice_id = key($aChecked[0]);

				$oShop_Warehouse_Invoice = Core_Entity::factory('Shop_Warehouse_Invoice')->getById($shop_warehouse_invoice_id);

				if (!is_null($oShop_Warehouse_Invoice))
				{
					$this->_oPrintlayout_Controller
						->replace($oShop_Warehouse_Invoice->getPrintlayoutReplaces())
						->driver($oPrintlayout_Driver)
						->entity($oShop_Warehouse_Invoice);
				}
			}
		}
