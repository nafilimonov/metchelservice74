...
/**
 * Shop_Warehouse_Invoice Backend Editing Controller.
 *
 * @package HostCMS
 * @subpackage Shop
 * @version 7.x
 ...
 */
class Shop_Warehouse_Invoice_Controller_Edit extends Admin_Form_Action_Controller_Type_Edit
{
	/**
	...
	protected function _prepareForm()
	{
		...
		$oSelect_Companies = Admin_Form_Entity::factory('Select')
			...
			->caption(Core::_('Shop_Warehouse_Invoice.company_id'))
		...
		if (Core::moduleIsActive('siteuser'))
		{
			...
			$oScriptSiteusers = Admin_Form_Entity::factory('Script')
				->value('
					$("#' . $windowId . ' #invoice_siteuser_company_id").select2({
			...
			$oSelectContracts = Admin_Form_Entity::factory('Select')
				...
				->caption(Core::_('Shop_Warehouse_Invoice.siteuser_company_contract_id'))
				->divAttr(array('class'=>'col-xs-12'));
		...
		$this->getField('registration_number')
			->divAttr(array('class' => 'form-group col-xs-6 col-sm-3 no-padding-right'))
			->add(
				Core_Html_Entity::factory('Span')
					->class('input-group-addon dimension_patch')
					->value(Core::_('Shop_Warehouse_Invoice.registration_date_preposition'))
			);

		$this->getField('registration_date')
			->divAttr(array('class' => 'form-group col-xs-6 col-sm-3 no-padding-left'));

		$oMainTab
			->move($this->getField('registration_number'), $oRegistrationRow)
			->move($this->getField('registration_date'), $oRegistrationRow);
		...
		$oShop_Warehouse_Select = Admin_Form_Entity::factory('Select')
			->caption(Core::_('Shop_Warehouse_Invoice.shop_warehouse_id'))
			->divAttr(
		...
		$oSelectResponsibleUsers = Admin_Form_Entity::factory('Select')
			...
			->caption(Core::_('Shop_Warehouse_Invoice.user_id'))
			->divAttr(array('class' => ''));
		...
		$oShop_Price_Select = Admin_Form_Entity::factory('Select')
			->caption(Core::_('Shop_Warehouse_Invoice.shop_price_id'))
			->divAttr(
				array('class' => 'form-group col-xs-12 col-sm-3')
			)
		...
		$oRecalcPriceLink
			...
			->a
				...
				->value(Core::_('Shop_Warehouse_Invoice.recalc_price'));
		$oRecalcPriceLink
			->icon
		...
		$oShopItemBlock
			->add(Admin_Form_Entity::factory('Div')
				...
				->value(Core::_('Shop_Warehouse_Invoice.shop_item_header'))
			)
		...
		$itemTable = '
			<div class="table-scrollable">
				<table class="table table-striped table-hover shop-item-table deals-aggregate-user-info">
					<thead>
						<tr>
							<th scope="col">' . Core::_('Shop_Warehouse_Invoice.position') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Invoice.name') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Invoice.measure') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Invoice.price') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Invoice.currency') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Invoice.quantity') . '</th>
							<th scope="col">' . Core::_('Shop_Warehouse_Invoice.sum') . '</th>
							<th scope="col"> </th>
						</tr>
					</thead>
					<tbody>
		...
		// $createFromDocument = $createFromPurchaseorder || $createFromSupply;
		...
		do {
			if ($createFrom && !$this->_object->id)
			{
				...
				{
					case 'purchaseorder':
						$oShop_Warehouse_Purchaseorder = Core_Entity::factory('Shop_Warehouse_Purchaseorder', $createFromId);
						$aShop_Warehouse_Invoice_Items = $oShop_Warehouse_Purchaseorder->Shop_Warehouse_Purchaseorder_Items->findAll();
					break;
					case 'supply':
						$oShop_Warehouse_Supply = Core_Entity::factory('Shop_Warehouse_Supply', $createFromId);
						$aShop_Warehouse_Invoice_Items = $oShop_Warehouse_Supply->Shop_Warehouse_Supply_Items->findAll();
					break;
				}
			}
			else
			{
				$oShop_Warehouse_Invoice_Items = $this->_object->Shop_Warehouse_Invoice_Items;
				$oShop_Warehouse_Invoice_Items->queryBuilder()
					->limit($limit)
					->offset($offset)
					->clearOrderBy()
					->orderBy('shop_warehouse_invoice_items.id');

				$aShop_Warehouse_Invoice_Items = $oShop_Warehouse_Invoice_Items->findAll(FALSE);
			}

			foreach ($aShop_Warehouse_Invoice_Items as $oShop_Warehouse_Invoice_Item)
			{
				$oShop_Item = Core_Entity::factory('Shop_Item')->getById($oShop_Warehouse_Invoice_Item->shop_item_id);
				...
				if (!is_null($oShop_Item))
				{
					...
					$onclick = !$createFrom || $this->_object->id
						? $oAdmin_Form_Controller->getAdminActionLoadAjax($oAdmin_Form_Controller->getPath(), 'deleteShopItem', NULL, 0, $oShop_Item->id, "shop_warehouse_invoice_item_id={$oShop_Warehouse_Invoice_Item->id}")
						: '';
					...
					$itemTable .= '
						<tr ' . ( !$createFrom ? ('id="' . $oShop_Warehouse_Invoice_Item->id) . '"' : '') . ' data-item-id="' . $oShop_Item->id . '">
							...
							<td width="110"><input type="text" class="price set-item-price form-control" name="shop_item_price' . (!$createFrom ? ('_' . $oShop_Warehouse_Invoice_Item->id) : '[]') . '" value="' . $oShop_Warehouse_Invoice_Item->price . '" /></td>
							<td>' . htmlspecialchars((string) $oShop_Item->Shop_Currency->sign) . '</td>
							<td width="80"><input class="set-item-count form-control" name="shop_item_quantity' . (!$createFrom ? ('_' . $oShop_Warehouse_Invoice_Item->id) : '[]')  . '" value="' . $oShop_Warehouse_Invoice_Item->count . '" /></td>
							<td><span class="calc-warehouse-sum">' . ($oShop_Warehouse_Invoice_Item->count * $oShop_Warehouse_Invoice_Item->price) . '</span></td>
							<td><a class="delete-associated-item" onclick="mainFormLocker.unlock(); res = confirm(\'' . Core::_('Shop_Warehouse_Invoice.delete_dialog') . '\'); if (res) { var next = $(this).parents(\'tr\').next(); $(this).parents(\'tr\').remove(); $.recountIndexes(next); ' . $onclick . ' } return res;"><i class="fa fa-times-circle darkorange"></i></a></td>
						</tr>
					';
				}
			}
		...
		while (!$createFrom && count($aShop_Warehouse_Invoice_Items));
		...
		$oShopItemRow1->add(
			Admin_Form_Entity::factory('Input')
				...
				->placeholder(Core::_('Shop_Warehouse_Invoice.add_item_placeholder'))
				->name('set_item_name')
		);
		...
		$this->title($this->_object->id
			? Core::_('Shop_Warehouse_Invoice.form_edit', $this->_object->number, FALSE)
			: Core::_('Shop_Warehouse_Invoice.form_add')
		);
	...
	public function execute($operation = NULL)
	{
		...
		// Создаем счет поставщика из формы редактирования заказа поставщику или приемки
		if ($createFromId && !is_null($oBaseDocument) && !$this->_object->id)
		{
			$oShopWarehouseInvoice = $oBaseDocument->getById($createFromId)->createShopWarehouseInvoice();

			$this->_object = $oShopWarehouseInvoice;
		}
		...
		{
			$sJsRefresh = Shop_Warehouse_Controller::getJsRefresh($this->_Admin_Form_Controller, $this->_object, array('supply'));
		}
	 ...
	 * @hostcms-event Shop_Warehouse_Invoice_Controller_Edit.onAfterRedeclaredApplyObjectProperty
	 */
	protected function _applyObjectProperty()
	{
		...
		// $iBaseDocumentId = $createFromPurchaseorder ? $createFromPurchaseorder : $createFromSupply;
		...
		{
			$modelName == 'shop_warehouse_invoice'
		...
		// Существующие товары
		$aShop_Warehouse_Invoice_Items = $this->_object->Shop_Warehouse_Invoice_Items->findAll(FALSE);
		foreach ($aShop_Warehouse_Invoice_Items as $oShop_Warehouse_Invoice_Item)
		{
			$oShop_Item = Core_Entity::factory('Shop_Item')->getById($oShop_Warehouse_Invoice_Item->shop_item_id);
			...
			if (!is_null($oShop_Item))
			{
				...
				$quantity = Core_Array::getPost('shop_item_quantity_' . $oShop_Warehouse_Invoice_Item->id, 0);

				$oShop_Warehouse_Invoice_Item->count != $quantity && $bNeedsRePost = TRUE;

				$oShop_Warehouse_Invoice_Item->count = $quantity;
				//$oShop_Warehouse_Purchaseorder_Item->price = $aPrices['price_tax'];
				$oShop_Warehouse_Invoice_Item->price = $quantity = Core_Array::getPost('shop_item_price_' . $oShop_Warehouse_Invoice_Item->id, 0);
		...
		if (count($aAddShopItems))
		{
			...
			foreach ($aAddShopItems as $key => $shop_item_id)
			{
				...
				if (!is_null($oShop_Item))
				{
					...
					$oShop_Warehouse_Invoice_Item
						->shop_warehouse_invoice_id($this->_object->id)
						->shop_item_id($oShop_Item->id)
						->count($count)
						->price($aPrices['price_tax'])
						->save();

					$onclick = 'mainFormLocker.unlock(); res = confirm(\'' . Core::_('Shop_Warehouse_Invoice.delete_dialog') . '\'); if (res) { var next = $(this).parents(\'tr\').next(); $(this).parents(\'tr\').remove(); $.recountIndexes(next); ' . $oAdmin_Form_Controller->getAdminActionLoadAjax($oAdmin_Form_Controller->getPath(), 'deleteShopItem', NULL, 0, $oShop_Item->id, "shop_warehouse_invoice_item_id={$oShop_Warehouse_Invoice_Item->id}") . ' } return res;';

					$script .= "aMapShopItemId['{$shop_item_id}'] = {
						id: {$oShop_Warehouse_Invoice_Item->id},
						onClickValue: \"{$onclick}\"
					};";

					$script .= "jShopItemPrice.eq({$key}).attr('name', 'shop_item_price_{$oShop_Warehouse_Invoice_Item->id}');
					jShopItemQuantity.eq({$key}).attr('name', 'shop_item_quantity_{$oShop_Warehouse_Invoice_Item->id}');";
				}
				else
				{
