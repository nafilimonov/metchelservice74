 ...
 */
class Lead_Controller_Timeline extends Admin_Form_Controller_View
{
	/**
	...
	protected function _showContent()
	{
		...
		?>
		<div>
			<form action="<?php echo Admin_Form_Controller::correctBackendPath('/{admin}/lead/timeline/index.php')?>?hostcms[action]=addNote&_=<?php echo time()?>&hostcms[checked][0][1-0]=1&lead_id=<?php echo $lead_id?>&parentWindowId=<?php echo htmlspecialchars($windowId)?>" method="POST" enctype='multipart/form-data' class="padding-bottom-10 dropzone-form dropzone-form-timeline">
						...
						foreach ($aTmpEntities as $key => $oTmpEntity)
						{
							...
							switch (get_class($oTmpEntity))
							{
								...
								case 'Crm_Note_Model':
									...
									{
										$files = $oTmpEntity->getFilesBlock($oTmpEntity->Lead);
									...
									$path = '/{admin}/lead/timeline/index.php';
								break;
								case 'Lead_Shop_Item_Model':
									$badge = 'fa fa-shopping-cart';
									$color = 'palegreen';

									$text = $oTmpEntity->showContent($oAdmin_Form_Controller);
								...
								case 'Lead_Step_Model':
									$badge = 'fa fa-retweet fa-rotate-90';

									// $text = $oTmpEntity->Lead_Status->name;

									$text = '<div><div class="deal-template-step-name deal-template-timeline" style="color: ' . Core_Str::hex2darker($oTmpEntity->Lead_Status->color, 0.2) . '; border-color: ' . $oTmpEntity->Lead_Status->color . '; background-color: ' . Core_Str::hex2lighter($oTmpEntity->Lead_Status->color, 0.88) . '">' . htmlspecialchars((string) $oTmpEntity->Lead_Status->name) . '</div></div>';

									$textColor = ' style="color: ' . $oTmpEntity->Lead_Status->color . ' !important"';
								break;
							...
							<li class="timeline-inverted <?php echo $class?>">
								...
								<div class="timeline-panel">
										...
										<div class="pull-right timeline-entity-actions">
											...
											if ($oEntity_Admin_Form->show_operations)
											{
												...
												foreach ($aAllowed_Admin_Form_Actions as $oAdmin_Form_Action)
												{
													...
													$href = $oAdmin_Form_Controller->getAdminActionLoadHref($path, $oAdmin_Form_Action->name, NULL, $datasetId, $key, $additionalParams, 10, NULL, NULL, NULL, 'list');

													$onclick = $oAdmin_Form_Action->name == 'edit'
														? $oAdmin_Form_Controller->getAdminActionModalLoad(array('path' => $path, 'action' => $oAdmin_Form_Action->name, 'operation' => 'modal', 'datasetKey' => $datasetId, 'datasetValue' => $key, 'additionalParams' => $additionalParams . '&parentWindowId=' . $parentWindowId, 'width' => '90%'))
														: $oAdmin_Form_Controller->getAdminActionLoadAjax($path, $oAdmin_Form_Action->name, NULL, $datasetId, $key, $additionalParams . '&parentWindowId=' . $parentWindowId, 10, NULL, NULL, NULL, 'list');
									...
									<div class="timeline-body">
										<span <?php echo $textColor?>><?php echo $text?></span>
										<div class="timeline-body-footer small gray">
											<?php
			...
			$form.dropzone({
				...
				init: function() {
					...
					$("#<?php echo $windowId?> .dropzone-form-timeline button#sendForm").on("click", function(e) {
						...
						{
							$form.append('<input type="hidden" name="hostcms[window]" value="<?php echo $windowId?>-lead-timeline">');
				...
				success : function(file, response){
					...
					$.adminLoad({ path: hostcmsBackend + '/lead/note/index.php', additionalParams: 'lead_id=<?php echo $lead_id?>', windowId: window_id });
				}
			});
		});
		</script><?php
