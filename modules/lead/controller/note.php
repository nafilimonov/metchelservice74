...
class Lead_Controller_Note extends Admin_Form_Controller_View
{
	...
	protected function _showContent()
	{
		...
		<div class="deal-note-board">
			...
			<div>
				<!-- <div class="col-xs-12"> -->
					<form action="<?php echo Admin_Form_Controller::correctBackendPath('/{admin}/lead/note/index.php')?>?hostcms[action]=addNote&_=<?php echo time()?>&hostcms[checked][0][0]=1&lead_id=<?php echo $lead_id?>&parentWindowId=<?php echo htmlspecialchars($windowId)?>" method="POST" enctype='multipart/form-data' class="padding-bottom-10 dropzone-form dropzone-form-note">
						<div class="timeline-comment-wrapper">
							<!-- <textarea rows="3" name="text_note" type="text" class="form-control" placeholder="<?php echo Core::_('Lead_Note.note_placeholder')?>"></textarea>-->
							<?php
					...
					$(function() {
						...
						$form.dropzone({
							...
							success : function(file, response){
								...
								$.adminLoad({ path: hostcmsBackend + '/lead/timeline/index.php', additionalParams: 'lead_id=<?php echo $lead_id?>', windowId: window_id });
							}
						});
					});
					</script>
							...
							<div class="timeline-panel">
										...
										if ($oAdmin_Form->show_operations)
										{
											...
											foreach ($aAllowed_Admin_Form_Actions as $oAdmin_Form_Action)
											{
												...
												$onclick = $oAdmin_Form_Action->name == 'edit'
													? "$.modalLoad({path: '{$oAdmin_Form_Controller->getPath()}', action: 'edit', operation: 'modal', additionalParams: 'hostcms[checked][0][{$oEntity->id}]=1&lead_id={$oLead->id}&parentWindowId={$windowId}', windowId: '{$windowId}', width: '90%'}); return false"
								...
								<div class="timeline-body">
									<?php
										...
										{
											$files = $oEntity->getFilesBlock($oLead);
