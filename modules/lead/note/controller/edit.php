 ...
 */
class Lead_Note_Controller_Edit extends Crm_Note_Controller_Edit
{
	/**
	 * Set object
	...
	public function setObject($object)
	{
		...
		$this->_relatedObject = Core_Entity::factory('Lead')->getById($iLeadId);
	...
	protected function _prepareForm()
	{
		...
		$oMainRow4->add(
			Admin_Form_Entity::factory('Code')
				->html('<input type="hidden" name="lead_id" value="' . $this->_relatedObject->id .'" />')
			);
	...
	{
		/*$iLeadtId = intval(Core_Array::getGet('lead_id'));

		$sJsRefresh = '<script>
			// Refresh lead notes list
			if ($("#id_content_timeline").length)
			{
				$.adminLoad({ path: hostcmsBackend + \'/lead/timeline/index.php\', additionalParams: \'lead_id=' . $iLeadtId . '\', windowId: \'id_content_timeline\' });
			}
			if ($("#id_content_notes").length)
			{
				$.adminLoad({ path: hostcmsBackend + \'/lead/note/index.php\', additionalParams: \'lead_id=' . $iLeadtId . '\', windowId: \'id_content_notes\' });
			}
		</script>';*/
