...
if ($oShopItemParent->id)
{
	...
	$href = Admin_Form_Controller::correctBackendPath("/{admin}/shop/item/index.php?shop_id={$oShop->id}&shop_group_id={$shop_group_id}&hostcms[checked][0][{$oShopItemParent->id}]=1");
	$onclick = Admin_Form_Controller::correctBackendPath("$.adminLoad({path: '/{admin}/shop/item/index.php', additionalParams: 'shop_id={$oShop->id}&shop_group_id={$shop_group_id}&hostcms[checked][1][{$oShopItemParent->id}]=1', action: 'edit', windowId: '{$windowId}'}); return false");
