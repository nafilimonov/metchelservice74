...
// Код формы
$iAdmin_Form_Id = 272;
$sAdminFormAction = '/{admin}/siteuser/note/index.php';
...
$oAdmin_Form_Controller
	...
	->title(Core::_('Siteuser_Note.siteuser_notes_title'))
	->pageTitle(Core::_('Siteuser_Note.siteuser_notes_title'))
	->Admin_View(
		Admin_View::getClassName('Admin_Internal_View')
	)
	->addView('note', 'Siteuser_Controller_Note')
	->view('note');
...
{
	$oSiteuser_Note_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Siteuser_Note_Controller_Edit', $oAdmin_Form_Action
	);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($oSiteuser_Note_Controller_Edit);
}
...
{
	$oSiteuser_Note_Controller_Markdeleted = Admin_Form_Action_Controller::factory(
		'Siteuser_Note_Controller_Markdeleted', $oAdmin_Form_Action
	);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($oSiteuser_Note_Controller_Markdeleted);
}
...
$oAdmin_Form_Dataset
	/*->addCondition(
		array('where' => array('siteuser_notes.siteuser_id', '=', $oSiteuser->id))
	)->addCondition(
		array('orderBy' => array('siteuser_notes.id', 'DESC'))
	)*/
	->addCondition(
		array('select' => array('crm_notes.*'))
	)
	->addCondition(
		array('leftJoin' => array('siteuser_crm_notes', 'crm_notes.id', '=', 'siteuser_crm_notes.crm_note_id'))
	)
	->addCondition(
		array('where' => array('siteuser_crm_notes.siteuser_id', '=', $oSiteuser->id))
	);
