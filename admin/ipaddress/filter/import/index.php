...
$ipaddress_filter_dir_id = Core_Array::getGet('ipaddress_filter_dir_id', 0);
...
$oAdmin_Form_Controller
	...
	->setUp()
	->path('/{admin}/ipaddress/filter/index.php');
...
$oAdmin_View
	->module(Core_Module_Abstract::factory($sModule))
	->pageTitle(Core::_('Ipaddress_Filter.import'));
...
$oAdmin_Form_Entity_Form = Admin_Form_Entity::factory('Form')
	...
	->action(Admin_Form_Controller::correctBackendPath('/{admin}/ipaddress/filter/index.php'))
...
$oAdmin_Form_Entity_Form->add($oMainTab
	->add(
		Admin_Form_Entity::factory('Div')->class('row')->add(Admin_Form_Entity::factory('File')
			->name("json_file")
			->caption(Core::_('Ipaddress_Filter.import_file'))
...
$oAdmin_Form_Entity_Form->add(
	...
	->name('show_form')
	->type('submit')
	->value(Core::_('Ipaddress_Filter.import_button'))
	->class('applyButton btn btn-blue')
	->onclick($oAdmin_Form_Controller->getAdminSendForm('importFilters', NULL, 'ipaddress_filter_dir_id=' . $ipaddress_filter_dir_id))
);
...
Core_Skin::instance()
	...
	->content(ob_get_clean())
	->title(Core::_('Ipaddress_Filter.import'))
	->execute();