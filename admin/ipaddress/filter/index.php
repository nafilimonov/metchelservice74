...
$oIpaddress_Filter_Dir = Core_Entity::factory('Ipaddress_Filter_Dir', Core_Array::getGet('ipaddress_filter_dir_id', 0));

$sFormTitle = $oIpaddress_Filter_Dir->id
	? $oIpaddress_Filter_Dir->name
	: Core::_('Ipaddress_Filter.title');
...
if (Core_Array::getPost('add_filter'))
{
	...
	Admin_Form_Entity::factory('Input')
		->name('header_name[]')
		->divAttr(array('class' => 'col-xs-12 col-sm-2 property-data ipaddress-filter-header-name hidden'))
		->placeholder(Core::_('Ipaddress_Filter.placeholder_name'))
		->controller($oAdmin_Form_Controller)
		->execute();
...
)
{
	...
	$aJSON[0] = array(
		'id' => 0,
		'label' => Core::_('Ipaddress_Filter_Dir.root'),
	);
	...
	if (strlen($sQuery))
	{
		...
		$oIpaddress_Filter_Dirs->queryBuilder()
			->where('ipaddress_filter_dirs.name', 'LIKE', $sQueryLike)
			->limit(Core::$mainConfig['autocompleteItems']);

		$aIpaddress_Filter_Dirs = $oIpaddress_Filter_Dirs->findAll(FALSE);

		foreach ($aIpaddress_Filter_Dirs as $oIpaddress_Filter_Dir)
		{
			$aParentDirs = array();

			$aTmpDir = $oIpaddress_Filter_Dir;
...
$additionalParams = "ipaddress_filter_dir_id={$oIpaddress_Filter_Dir->id}";
...
$oAdmin_Form_Entity_Menus->add(
	...
	Admin_Form_Entity::factory('Menu')
		->name(Core::_('Ipaddress_Filter.add_group'))
		->icon('fa fa-plus')
		->href(
...
$oAdmin_Form_Entity_Breadcrumbs->add(
	...
	Admin_Form_Entity::factory('Breadcrumb')
		->name(Core::_('Ipaddress_Filter.title'))
		->href(
...
// Крошки по группам
if ($oIpaddress_Filter_Dir->id)
{
	$oIpaddress_Filter_Dir_Breadcrumbs = $oIpaddress_Filter_Dir;
	...
	if (!$oUserCurrent->read_only)
	{
		if (isset($_FILES['json_file']) && intval($_FILES['json_file']['size']) > 0)
		{
			try {
				...
				$oIpaddress_Filter_Import_Controller = Admin_Form_Action_Controller::factory(
					'Ipaddress_Filter_Import_Controller', $oAdmin_Form_Action
				);

				$oIpaddress_Filter_Import_Controller
					->content($content)
					->ipaddress_filter_dir_id($oIpaddress_Filter_Dir->id)
					// ->execute()
					;

				$oAdmin_Form_Controller->addAction($oIpaddress_Filter_Import_Controller);
			}
			catch (Exception $exc) {
...
{
	$oIpaddress_Filter_Export_Controller = Admin_Form_Action_Controller::factory(
		'Ipaddress_Filter_Export_Controller', $oAdminFormActionExportFilters
	);

	$oIpaddress_Filter_Export_Controller
		->controller($oAdmin_Form_Controller)
		->export();
}
...
{
	$Ipaddress_Filter_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Ipaddress_Filter_Controller_Edit', $oAdmin_Form_Action
	);
	...
	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($Ipaddress_Filter_Controller_Edit);
}
...
if ($oAdminFormActionMove && $oAdmin_Form_Controller->getAction() == 'move')
{
	...
	$iCount = Core_Entity::factory('Ipaddress_Filter_Dir')->getCount();
	...
	if ($iCount < Core::$mainConfig['switchSelectToAutocomplete'])
	{
		...
		$Ipaddress_Filter_Controller_Edit = new Ipaddress_Filter_Controller_Edit($oAdminFormActionMove);

		$Admin_Form_Action_Controller_Type_Move
			// Список директорий генерируется другим контроллером
			->selectOptions(array(' … ') + $Ipaddress_Filter_Controller_Edit->fillIpaddressFilterDir(0, $aExclude));
	}
	else
	{
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Ipaddress_Filter_Dir')
);
...
if (strlen($sGlobalSearch))
{
	$oAdmin_Form_Dataset
		->addCondition(array('open' => array()))
			->addCondition(array('where' => array('ipaddress_filter_dirs.id', '=', is_numeric($sGlobalSearch) ? intval($sGlobalSearch) : 0)))
			->addCondition(array('setOr' => array()))
			->addCondition(array('where' => array('ipaddress_filter_dirs.name', 'LIKE', '%' . $sGlobalSearch . '%')))
		->addCondition(array('close' => array()));
}
else
{
	$oAdmin_Form_Dataset->addCondition(array('where' => array('parent_id', '=', $oIpaddress_Filter_Dir->id)));
}
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Ipaddress_Filter')
);
...
if (strlen($sGlobalSearch))
{
	$oAdmin_Form_Dataset
		->addCondition(array('open' => array()))
			->addCondition(array('where' => array('ipaddress_filters.id', '=', is_numeric($sGlobalSearch) ? intval($sGlobalSearch) : 0)))
			->addCondition(array('setOr' => array()))
			->addCondition(array('where' => array('ipaddress_filters.name', 'LIKE', '%' . $sGlobalSearch . '%')))
		->addCondition(array('close' => array()));
}
else
{
	// Ограничение источника 1 по родительской группе
	$oAdmin_Form_Dataset->addCondition(array('where' => array('ipaddress_filter_dir_id', '=', $oIpaddress_Filter_Dir->id)));
}
