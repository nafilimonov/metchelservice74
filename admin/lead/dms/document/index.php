...
// Код формы
$iAdmin_Form_Id = 334;
$sAdminFormAction = '/{admin}/lead/dms/document/index.php';
...
$oAdmin_Form_Controller
	...
	->title(Core::_('Lead_Dms_Document.title'))
	->pageTitle(Core::_('Lead_Dms_Document.title'))
	->Admin_View(
		Admin_View::getClassName('Admin_Internal_View')
	)
	->addView('view', 'Lead_Dms_Document_Controller_View')
	->view('view');
...
$oAdmin_Form_Dataset
	...
	->addCondition(
		array('join' => array('lead_dms_documents', 'lead_dms_documents.dms_document_id', '=', 'dms_documents.id'))
	)
	->addCondition(
		array('where' => array('lead_dms_documents.lead_id', '=', $oLead->id))
	)
	->addCondition(
		array('orderBy' => array('lead_dms_documents.id', 'DESC'))
	);
